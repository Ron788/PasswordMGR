...
class StorageServiceTest {
    ...
    void writeRead() throws IOException {
        String dataToSave = "Test. Раскольников не привык к толпе и, как уже сказано, бежал всякого общества, особенно в последнее время. Но теперь его вдруг что-то" +
                ...
                "а всё лицо его было как будто смазано маслом, точно железный замок. За застойкой находился мальчишка лет четырнадцати," +
        ...
        StorageService storageService = new StorageService("./testStorage/");

        storageService.write(dataToSave);
        String result = storageService.read();
